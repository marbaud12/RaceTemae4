...
```{r}
# Para observar la variable cran, ponemos la variable tal cual
cran

glimpse(cran)
# El glimpse lo muestra en horizontal y sin formato tabla
```
# dplyr
...
```{r}
# Usaremos select y los dos puntos
...
# Usaremos la barra vertical, equivalente al 'or'
```
...
```{r}
# Para que no este vacia podemos usar el is.na con el signo de exclamación
...
```{r}
arrange(cran2, ip_id)
# Como es en orden ascendente no es necesario indicar nada, ya que por defecto
# lo ordena de manera ascendente
```
12. Ordena `cran2` en orden descendente según la variable `ip_id`.
```{r}
# En este caso si que tendremos que indicar desc
...
```{r}
cran3 <- mutate(cran3, size_mb = size / (2^10))
...
top_counts_sorted <- pack_sum %>% 
  filter(count>umbral) %>% arrange(desc(count))
